...
Double-buffered VGA ensures tear-free animation.
...
Scancode register and status bit
...
Buffer swap control & status

PID Sensor Input

0xFF200080

8-bit process variable

PID Output

0xFF200000

Shares LED bus for demonstration

Adjust addresses in code if your platform differs.

Building

Import into your FPGA toolchain (Quartus, Vivado, etc.) and assign the peripheral base addresses.

Compile this C code with your Nios II / MicroBlaze SDK:

nios2-bsp-generate --app csrc robot_arm_controller
make

Load the ELF onto the soft CPU and run.

Usage

Increase/Decrease Segments

Timer 1 (Segment 1): A / Z

Timer 2 (Segment 2): S / X

Timer 3 (Segment 3): D / C

Timer 4 (Segment 4): F / V

Timer 5 (Claw)     : G / B

Hold any key for accelerated adjustment.

Watch the VGA screen to see updated angle values and real-time arm rendering.

PID Tuning

Modify the pid_init() call in main() to set:

    pid_init(&pid, Kp, Ki, Kd, setpoint);

Recompile to apply new PID gains or target setpoint.
